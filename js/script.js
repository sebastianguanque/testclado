...
// Elementos del DOM
const palabrasValueSpan = document.querySelector(
  ".tile__palabras .tile__value"
);
const precisionValueSpan = document.querySelector(
  ".tile__precision .tile__value"
);
...
class Game {
  ...
  setEventListeners() {
    ...
    this.textAreaInput.addEventListener("keydown", (e) => {
      if (e.key === "Backspace") {
        e.preventDefault(); // Muy importante para prevenir el borrado nativo del input oculto
        this.handleBackspace();
        this.updateCursorPosition();
      } else if (e.key === "Enter") {
        e.preventDefault(); // Muy importante para prevenir comportamientos por defecto del Enter
        this.handleEnter();
        this.updateCursorPosition();
      }
    });
  ...
  handleTextInput(event) {
    ...
    // Queremos procesar cada caracter ingresado
    if (typedText.length > 0) {
      const typedKey = typedText.charAt(typedText.length - 1); // Obtiene el último caracter ingresado

      // Limpia el input inmediatamente después de capturar el caracter
      // para que esté listo para la siguiente entrada y no acumule texto.
      event.target.value = "";

      // Si el caracter es válido, lo procesamos.
      // Se mantiene la validación para asegurar que solo se procesen caracteres de escritura
  ...
  handleKeyDown(event) {
    ...
    // Ignorar ciertas teclas que no queremos que afecten el juego
    ...
    if (key === "Backspace") {
      event.preventDefault(); // Previene que el navegador borre el contenido del input oculto
      this.handleBackspace();
      this.updateCursorPosition();
    }
